    ...
    public boolean play(int step, ParticleEffectLocationCalculator locationCalculator, Player player) {
        ...
        for(ParticleEffectComponent component: components) {
            if(component.isActive(localStep)) {
                ...
                for(int timelineNum = 0; timelineNum < component.getActiveTimelineCount(localStep); timelineNum++) {

                    Location location = locationCalculator.getLocationForStep(step - component.getLocationOffset(localStep, timelineNum));
                    ...
                    for(Vector vec: component.getModifiedCoordinates(localStep, false)) { // TODO there should be an option to have the timeline offset on the localStep too
